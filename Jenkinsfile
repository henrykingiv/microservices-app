pipeline { 
    ...
    environment {
        ...
        DOCKER_IMAGE = 'henrykingiv/recommendationservice'
        TARGET_BRANCH = 'main'
    ...
    stages {
            ...
            steps {
                script {
                    ...
                    withCredentials([usernamePassword(credentialsId: 'git-creds', usernameVariable: 'GIT_USERNAME', passwordVariable: 'GIT_PASSWORD')]) {
                        sh "git pull https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/henrykingiv/microservices-app.git HEAD:main"
        ...
        stage('Update Prod Manifest File') {
            steps {
                script {
                    def manifestFile = "deployment-service.yaml"
                    def majorVersion = '1'
                    ...
                    // Push the changes
                    withCredentials([usernamePassword(credentialsId: 'git-creds', usernameVariable: 'GIT_USERNAME', passwordVariable: 'GIT_PASSWORD')]) {
                        sh "git push https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/henrykingiv/microservices-app.git HEAD:production"
                    }
                }
            }
        }
